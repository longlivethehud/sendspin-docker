...
```
docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v8,linux/arm/v7 --build-arg VERSION="5.2.1" --build-arg BUILD_DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ") --no-cache-filter upgrade-sendspin -f Dockerfile -t ghcr.io/longlivethehud/sendspin-docker:latest --pull --push .
```
